...
public interface class IremotecontrolvehicleConsumer
{
};

public ref class remotecontrolvehicleConsumer sealed  : [Windows::Foundation::Metadata::Default] IremotecontrolvehicleConsumer, ISignalEmitter
{
public:
    remotecontrolvehicleConsumer(Windows::Devices::AllJoyn::AllJoynBusAttachment^ busAttachment);
    virtual ~remotecontrolvehicleConsumer();
    ...
    static Windows::Foundation::IAsyncOperation<remotecontrolvehicleConsumer^>^ FromIdAsync(_In_ Platform::String^ deviceId);

    // Create a consumer from a device Id asynchronously with the provided bus attachment.
    //
    // This is usually called to create a consumer after the unique name of a producer has been reported
    // in the Added callback on the DeviceWatcher.
    static Windows::Foundation::IAsyncOperation<remotecontrolvehicleConsumer^>^ FromIdAsync(_In_ Platform::String^ deviceId, _In_ Windows::Devices::AllJoyn::AllJoynBusAttachment^ busAttachment);

    int32 remotecontrolvehicleConsumer::Initialize(_In_ Windows::Devices::AllJoyn::AllJoynServiceInfo^ serviceInfo);

    // Call the GetAnalogChannelData method
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleGetAnalogChannelDataResult^>^ GetAnalogChannelDataAsync(_In_ uint32 interfaceMemberChannelId);
    // Call the SetAnalogChannelState method
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleSetAnalogChannelStateResult^>^ SetAnalogChannelStateAsync(_In_ uint32 interfaceMemberChannelId, _In_ double interfaceMemberValue);
    // Call the SetToggleChannelState method
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleSetToggleChannelStateResult^>^ SetToggleChannelStateAsync(_In_ uint32 interfaceMemberChannelId, _In_ uint32 interfaceMemberValue);
    // Call the SetMultipleAnalogChannelStates method
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleSetMultipleAnalogChannelStatesResult^>^ SetMultipleAnalogChannelStatesAsync(_In_ Windows::Foundation::Collections::IVectorView<uint32>^ interfaceMemberChannelIds, _In_ Windows::Foundation::Collections::IVectorView<double>^ interfaceMemberValues);

    // Get the value of the ReceiverName property.
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleGetReceiverNameResult^>^ GetReceiverNameAsync();

    // Get the value of the Manufacturer property.
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleGetManufacturerResult^>^ GetManufacturerAsync();

    // Get the value of the DeviceType property.
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleGetDeviceTypeResult^>^ GetDeviceTypeAsync();

    // Get the value of the Channels property.
    Windows::Foundation::IAsyncOperation<remotecontrolvehicleGetChannelsResult^>^ GetChannelsAsync();


    // Used to send signals or register functions to handle received signals.
    property remotecontrolvehicleSignals^ Signals
    {
        remotecontrolvehicleSignals^ get() { return m_signals; }
    }
...
private:
    ...
    remotecontrolvehicleSignals^ m_signals;
